        ...
        # Botão de Adicionar notas
            ...
            if titulo != "":
                ...
                if selected_date < datetime.now().date():
                    sg.popup("Você não pode adicionar um evento com data no passado!", button_color="#4169E1")
                else:
                    titulo = values["-EVENTO-"]
...
def cronômetro():
    ...
    buttons_layout = [
        [sg.Button("INICIAR", key='-START-', button_color="#4169E1", size=(11, 2)),
         sg.Button("PAUSAR", key='-PAUSE-', button_color="#4169E1", size=(11, 2)),
...
def editar_perfil():
    ...
    window = sg.Window("Editar Perfil", layout, size=(400, 216))
    button, values = window.read()
    window.close()
